...
groupadd -g 2017 $USERNAME
useradd  -u 2017 -g $USERNAME -s /bin/false $USERNAME
...
sed -i 's/^Server=.*/'"Server=$SerIP"'/' $ZABBIX_AGENT_CONF
sed -i 's/^ServerActive=.*/'"ServerActive=$SerIP"'/' $ZABBIX_AGENT_CONF
